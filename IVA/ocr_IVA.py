...
def val_iva(text):
    # Patrón de expresión regular insensible a mayúsculas/minúsculas y tildes
    pattern = re.compile(r'\bTotal del Débito Fiscal|Total del débito fiscal del período|Total del débito tiscal del periodo|Total del débito tiscal del período|del débito fiscal clel periodo|el débito fiscal clel periodo\b', re.IGNORECASE)
...
def val_period_810(text):
    # Patrón de expresión regular insensible a mayúsculas/minúsculas y tildes
    pattern = re.compile(r'p[eé]r[ií][oó]d[oó]\s+f[ií]sc[aá]l', re.IGNORECASE)
...
def val_period_2002(text):
    # Patrón de expresión regular insensible a mayúsculas/minúsculas y tildes
    pattern = re.compile(r"per[ií]odo:\s*(\d+)", re.IGNORECASE)
...
def contains_cuit_2002(text):
    # Patrón para buscar el cuit
    pattern = r'CUIT\s*(?:N[ºo\'\"]|NP)[:;]?\s*(\d{11})'
    # Buscar el patrón en el texto
    result = re.search(pattern, text,re.IGNORECASE)
...
def run(id_entrada, sql_connector):
    # Obtener la ubicación actual
    ubicacion_actual = os.getcwd()

    # Ruta a la carpeta que contiene los archivos PDF
    #carpeta_pdf = os.path.join(ubicacion_actual, 'PDF IVA/formato810/DDJJ de IVA')
    print(ubicacion_actual)
    carpeta_pdf = os.path.join(ubicacion_actual, 'MLPA\IVA')
    ...
    with os.scandir(carpeta_pdf) as carpetas_bancos:
        for fichero in carpetas_bancos:
            ...
            print(f'ruta final :{carpeta_banco}')
            entidad = fichero
            ...
            server = 'ec2-18-223-20-191.us-east-2.compute.amazonaws.com'
            database = 'BBDD_MLAP'
            user = 'sa'
            password = r'#MSApprating#'
            ...
            for pdf_IVA_index,archivo_pdf in enumerate(archivos_en_carpeta):
                ...
                for index_page, page in enumerate(pages):
                    ...
                    for i, columna in enumerate(columnas_texto):
                        ...
                        if Format =='2002':
                            for j, detection in enumerate(columna):
                                ...
                                #extraer periodo    
                                if val_period_2002(text):
                                    numero_ext = str(val_period_2002(text)).replace("/","")
                    ...
                    if (Iva_leido==True or (index_page+1==num_pages)):
                        ...
                        print(f"-------------------")
                        insert_statement = "Insert into TL_IVA(id_ejecucion,Banco,CUIT, Nombre_documento, Periodo, Valor_IVA_leido, Confianza, formato,estado,I_sub_estado_proceso,observacion,fecha_creacion) " \
