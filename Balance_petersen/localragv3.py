            ...
            for fichero in carpetas_bancos:
                ...
                for index,archivo_pdf in enumerate(archivos_en_carpeta):
                    if archivo_pdf.endswith('.PDF') or archivo_pdf.endswith('.pdf'):
                        ...
                        procesar_llm(id_lote,id_doc,cuit,pdf_path,pdf_path_out,nombre_doc,ubicacion_actual,sql_connector,args)
    else:
...
def procesar_llm(id_lote,id_doc,cuit,pdf_path,pdf_path_out,nombre_doc,ubicacion_actual,sql_connector,args):
    ...
    if os.path.exists(pdf_path):
        ...
        if vault_content:
           ...
           # periodo_procesado=data["estado_de_situacion_financiera"][0]["fecha"]
            ...
            #Consulta cliente 
            cuit,id_cliente,nombre_cliente = consultar_cliente(cuit,sql_connector)
...
def procesar_fila(row, index, id_doc, id_balance_max, categoria_diccionario, diccionario_resultantes,categoria):
    ...
    # Retornar los queries generados
    
def consultar_cliente(cuit,sql_connector):
